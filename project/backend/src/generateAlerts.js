...
// Simple rules-based alert/advice generator based on stored forecasts
...
async function generateAlertsForRegion(regionId) {
    ...
    for (const f of forecasts) {
        ...
        // Decide rules (expanded to be more permissive)
        const alerts = [];

        // Heat: more granular thresholds
        if (f.temp_max != null) {
        ...
        // Rain detection from condition text (existing simple heuristic)
        if (f.condition && /rain|Rain|Orana|orana|drizzle|Drizzle/.test(f.condition)) {
            // heavy-sounding words -> warning, otherwise info
        ...
        // Guarantee at least one informational alert per day if no alerts generated
        if (alerts.length === 0) {
            alerts.push({ type: 'weather', severity: 'info', value: `${f.temp_min ?? '-'} / ${f.temp_max ?? '-'}` });
        }

        for (const a of alerts) {
            // remove similar existing alerts for same region/date/type to keep idempotent
